...
def get_seperate_areas(inputs, target): #liu
...
def valid_test(net, batch_size, fun_type = 'valid', rebuild = False, **params_dic):
	net.eval()
	if fun_type == 'valid':
	...
	for number_image in range(image_number):
		unit_image = np.load('../../final_dataset/'+fun_type+'_image/'+str(number_image)+'.npy')
		unit_label = np.load('../../final_dataset/'+fun_type+'_label/'+str(number_image)+'.npy').astype(np.float32)
...
def train_net(net, epochs = 100, batch_size = 12, lr = 1e-4, gpu = True, rebuild = False):
	...
	train_dataset = yield_data()
	if not rebuild:
		search_dataset = yield_data(search = True)
	for epoch in range(epochs):
		if epoch % 3 == 0:
			highest_dice = valid_test(net, batch_size, epoch = epoch, fun_type = 'valid', 
...
def test_net(net, save_file_name, rebuild = False, batch_size = 12, gpu = True, pth_name = 'SearchSave.pth'):
	print('''
		Starting testing:
		Batch size: {}
		CUDA: {}
	...
	return valid_test(net, batch_size, fun_type = 'test', load_checkpoint = pth_name, save_file_name = save_file_name, rebuild = rebuild)
